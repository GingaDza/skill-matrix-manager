...
class DatabaseManager:
    def __init__(self):
        ...
        self.current_time = "2025-02-07 12:36:38"  # 現在の時刻
    ...
    def _insert_initial_data(self):
        ...
        try:
            # デフォルトグループの作成
            self.cursor.execute('''
                INSERT OR IGNORE INTO groups (name, created_at, created_by)
                VALUES (?, ?, ?)
            ''', ('デフォルトグループ', self.current_time, self.current_user))

            # デフォルトカテゴリーの作成
            self.cursor.execute('''
                INSERT OR IGNORE INTO categories (name, parent_id, is_skill, created_at, created_by)
                VALUES (?, NULL, 0, ?, ?)
